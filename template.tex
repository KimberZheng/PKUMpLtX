...
在此列举几个比较重要的参数：learning\_rate 设置为 5e-5，适合小样本微调；batch\_size 设置为 2，结合梯度累积（gradient\_accumulation\_steps=8）可以模拟大批次训练；
训练的epochs 设置为 5，由于参数比较少，可以防止过拟合。
...
\subsection{我的分工}
在这部分中，我负责了部分数据集的人工收集整理，并且辅助了一些模型部署的过程，例如如何具体部署Ollama、Fay、和UE5的资料查找，语音识别模块API的接入，
并负责最后成果报告的展示。
