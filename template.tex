...
\section{结果及讨论}
  ...
  \subsection{观察掺有三氯化铁水样品的核磁共振现象}
    ...
    外加总磁场为\( B = B_0 + B' \cos \omega t \), 其中前者为扫场, 后者为射频场. 

    若固定扫场幅度, 调节射频场频率, 即改变\autoref{fig:yuanli}中\(2\pi\nu/\gamma\)和总磁场 B 的交点, 
    则会改变两个共振峰之间的距离, 从而使得示波器上相邻共振峰两两接近, 最终合并为一个. 

    若固定射频场频率, 调节扫场幅度, 即改变\autoref{fig:yuanli}中\( B_0 \pm B' \)的位置. 随着\(B_0\)的不断增大, 
    \(2\pi\nu/\gamma\)和总磁场 B 的交点从无到有, 相邻两交点的逐渐分开, 再和另一个相邻的交点接近, 最终再合并. 

    改变电路盒位置, 相当于改变了\( B_0 \)的均匀性, 而不均匀的磁场会减小样品的横向弛豫时间, 导致尾波衰减更快, 从而影响了共振峰的高度和尾波数量. 经过观察, 磁场最均匀的地方在电路盒右边缘位置\(x = 0.70cm\)处. 

    改变射频场幅度, 对示波器图像的影响较小, 因为改变射频场幅度对峰间距影响不大, 而只会影响峰的高度. 
    

  \subsection{校准永磁铁的磁场}
    将样品置于磁场均匀性最好的位置\(x = 0.70cm\), 找到共振信号后尽可能减小扫场幅度, 这样可以减小共振频率测量误差. 
    减小扫场幅度为15.2格, 调整射频场频率使得示波器上出现多个共振峰, 峰与峰之间间隔为 10ms 左右, 记录下此时的射频场频率\( \nu_H = 21.10232 \, \text{MHz} \). 
    ...
    记录下满足该情形的频率分别为 \( \nu' = 21.08453 \, \text{MHz} \) 和 \( \nu'' = 21.11984\, \text{MHz} \). 由此得到: 
...
\section{结论}
  ...
  $g = 5.2523 \pm 0.0007$,  最后对氟核的横向弛豫时间进行了测量, 得到结果为$ T_2 = 36.1995 \mathrm{ \mu s}$.
